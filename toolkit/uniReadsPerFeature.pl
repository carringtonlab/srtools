...
my (%opt, @list, $outfile, $species, $confFile, @range, $gff, %sizes, %libs, %hits);
...
print OUT "Feature\tSenseSeqs\tAntiSeqs\tSenseReads\tAntiReads\tLength\tAnnotation\n";
...
while (my $line = <GFF>) {
	...
	print OUT $feature;
	print OUT "\t".$seqs{'sense'}."\t".$seqs{'anti'};
