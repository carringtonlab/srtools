...
my (%opt, $outfile, $species, $confFile, @libs, %reads, %unapproved, @sizes, %sizes, $sth, $region, @hits, $color_mode, $totalReads);

#my $totalReads = 0;
my $ntwidth = 10;

getopts('o:S:s:C:l:c:R:m:t:h', \%opt);
var_check();
...
my $iwidth = ($obs_end - $rstart + 1) * $ntwidth;
my $iheight = 100;
...
sub var_error() {
	...
	print STDERR "  Usage:\n";
	print STDERR "  plotRegionProcessingAC_sameScale.pl -S <species> -c <conf file> -l <library ID(s)> -o <output file prefix> -R <region> -t <total reads>\n";
	print STDERR "\n\n";
	...
	print STDERR "\n";
	print STDERR "  -t   The total read denominator. Used to set the same y-axis scaling for multiple graphs.\n";
	print STDERR "\n";
